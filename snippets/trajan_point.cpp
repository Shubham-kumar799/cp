...
// ap marks articulation points
void dfs(int node, int parent, vb &vis, vi &low, vi &discover, mivi &graph, vb &ap)
{
  discover[node] = low[node] = cnt++;
  vis[node] = 1;
  int child = 0;
  for (auto nbr : graph[node])
  {
    ...
    if (vis[nbr])
    {
      low[node] = min(low[node], discover[nbr]);
      continue;
    }
    child++;
    dfs(nbr, node, vis, low, discover, graph, ap);
    low[node] = min(low[node], low[nbr]);
    if (discover[node] <= low[nbr] && parent != -1)
      ap[node] = 1;
  }
  if (child > 1 && parent == -1)
    ap[node] = 1;
